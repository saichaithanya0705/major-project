...
function getChatSessionState(preferredSessionId = null) {
  return chatSessions.getChatSessionState(preferredSessionId);
}

function loadChatSessionData(sessionId = null) {
  return chatSessions.loadChatSessionData(sessionId);
}

function saveChatSessionMessages(sessionId, messages) {
  return chatSessions.saveChatSessionMessages(sessionId, messages);
}
...
function createChatSession(setAsCurrent = true) {
  return chatSessions.createChatSession(setAsCurrent);
}
...
ipcMain.handle('save-chat-session', async (event, payload) => {
  return saveChatSessionMessages(payload?.sessionId || chatSessions.getCurrentSessionId(), payload?.messages || []);
});
